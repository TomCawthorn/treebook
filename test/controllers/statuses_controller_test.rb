...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should not display a blocked users post when loggin in" do
    sign_in users(:tom)
    users(:blocked_friend).statuses.create(content: 'Blocked status')
    users(:john).statuses.create(content: 'Okay status')
    get :index
    assert_match /Okay\ status/, response.body
    assert_no_match /Blocked\ status/, response.body
  end

  test "should not display users post when not logged in" do
    users(:blocked_friend).statuses.create(content: 'Blocked status')
    users(:john).statuses.create(content: 'Okay status')
    get :index
    assert_match /Okay\ status/, response.body
